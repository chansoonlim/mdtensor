...
struct Target {
    ...
    static inline void run(benchmark::State &state) {
        ...
        const auto in1 = md::ones<dtype>(md::dims<2>{len, len});
        const auto in2 = md::ones<dtype>(md::dims<1>{len});
        auto out = md::empty<dtype>(len);

        benchmark::ClobberMemory();

        for (auto _ : state) {
            static_cast<void>(md::linalg::matvec<void, backend>(in1, in2, out));
            benchmark::ClobberMemory();
        }
...
int main(int argc, char **argv) {
    ...
    settings.benchmark_name = "matvec";

...
TEST(run_time, 1) {
    const auto out = md::multiply(2.0, 4.0);
...
TEST(run_time, 2) {
    ...
    const auto out = md::multiply(x1, x2);
...
TEST(compile_time, 1) {
    constexpr auto out = md::multiply(2.0, 4.0);

    std::cout << "out: " << out << std::endl;

    static_assert(out == 8.0);
}
...
TEST(compile_time, 2) {
    ...
    constexpr auto out = md::multiply(x1, x2);

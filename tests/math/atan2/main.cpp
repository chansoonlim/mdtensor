...
TEST(run_time, 1) {
    ...
    const auto x =
        md::container<int, md::dims<1>>{{-1, 1, 1, -1}, md::dims<1>{4}};
    const auto y =
        md::container<int, md::dims<1>>{{-1, -1, 1, 1}, md::dims<1>{4}};
    ...
    ASSERT_TRUE(
        md::allclose(out, md::container<double, md::extents<index_t, 4>>{
                              {-135., -45., 45., 135.}}));
}
...
TEST(run_time, 2) {
    ...
    const auto y =
        md::container<value_t, md::dims<1>>{{1., -1.}, md::dims<1>{2}};
    const auto x =
        md::container<value_t, md::dims<1>>{{0., 0.}, md::dims<1>{2}};
    const auto out = md::atan2(y, x);

    std::cout << "out: " << md::to_string(out) << std::endl;

    ASSERT_TRUE(
        md::allclose(out, md::container<value_t, md::extents<index_t, 2>>{
...
TEST(run_time, 3) {
    ...
    const auto y = md::container<value_t, md::dims<1>>{
    ...
    ASSERT_TRUE(
        md::allclose(out, md::container<value_t, md::extents<index_t, 3>>{
...
TEST(compile_time, 1) {
    ...
    constexpr auto x =
        md::container<int, md::extents<index_t, 4>>{{-1, 1, 1, -1}};
    constexpr auto y =
        md::container<int, md::extents<index_t, 4>>{{-1, -1, 1, 1}};
...
TEST(compile_time, 2) {
    ...
    constexpr auto y =
        md::container<value_t, md::extents<index_t, 2>>{{1., -1.}};
    constexpr auto x =
        md::container<value_t, md::extents<index_t, 2>>{{0., 0.}};
...
TEST(compile_time, 3) {
    ...
    constexpr auto y = md::container<value_t, md::extents<index_t, 3>>{

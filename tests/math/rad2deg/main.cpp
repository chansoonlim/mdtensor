...
TEST(stack, rad2deg) {
    ...
    constexpr auto a = md::container<T, md::extents<size_t, 3>>{
        {0, std::numbers::pi_v<T> / 2., std::numbers::pi_v<T>}};
    constexpr auto b = md::rad2deg(a);

    constexpr auto b_expect =
        md::container<T, md::extents<size_t, 3>>{{0, 90, 180}};
...
TEST(heap, rad2deg) {
    ...
    const auto a = md::container<T, md::dims<1>>{
        {0, std::numbers::pi_v<T> / 2., std::numbers::pi_v<T>}, md::dims<1>{3}};
    const auto b = md::rad2deg(a);

    const auto b_expect =
        md::container<T, md::dims<1>>{{0, 90, 180}, md::dims<1>{3}};

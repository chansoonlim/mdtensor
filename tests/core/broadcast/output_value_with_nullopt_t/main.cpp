...
using test_t = typename md::core::output_value_with_nullopt_t<dtype, Ts...>;
...
TEST(test, 1) {
    static_assert(std::same_as<test_t<void, int, std::nullopt_t>, int>);
}

TEST(test, 3) {
    const std::nullopt_t a = std::nullopt;

    const auto a_mds = md::core::to_mdspan(a);

    using a_mds_t = decltype(a_mds);

    // static_assert(std::same_as<test_t<void, int, a_mds_t>, int>);
}

...
TEST(run_time, 2) {
    ...
    const auto a = md::tensor<value_t, md::dims<1>>{{1, 2}, md::dims<1>{2}};
    const auto b =
        md::tensor<value_t, md::dims<2>>{{1, 3, 1, 4}, md::dims<2>{2, 2}};
    const auto c = md::not_equal(a, b);

    EXPECT_TRUE(md::array_equal(c, md::tensor<bool, md::extents<index_t, 2, 2>>{
...
TEST(compile_time, 2) {
    ...
    constexpr auto a = md::tensor<value_t, md::extents<index_t, 2>>{{1, 2}};
    constexpr auto b =
        md::tensor<value_t, md::extents<index_t, 2, 2>>{{1, 3, 1, 4}};
    constexpr auto c = md::not_equal(a, b);

    static_assert(
        md::array_equal(c, md::tensor<bool, md::extents<index_t, 2, 2>>{
                               {false, true, false, true}}));
}

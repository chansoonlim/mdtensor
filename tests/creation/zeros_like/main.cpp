...
TEST(run_time, 1) {
    ...
    EXPECT_TRUE(md::array_equal(
        z,
        md::tensor<value_t, md::extents<index_t, 2, 3>>{{0, 0, 0, 0, 0, 0}}));
}
...
TEST(run_time, 2) {
    ...
    EXPECT_TRUE(md::array_equal(
        z, md::tensor<value_t, md::extents<index_t, 3>>{{0, 0, 0}}));
}
...
TEST(compile_time, 1) {
    ...
    constexpr auto z = md::zeros_like(x);

...
//BEGIN_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/base/tensor.hpp
/**
 * @file
...
//END_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/base/tensor.hpp
...
//BEGIN_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/tensor/resolve_broadcasted_output.hpp
/**
 * @file
 * @brief Output utilities for mdtensor.
 *
...
//BEGIN_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/tensor/resolve_output.hpp
/**
 * @file
 * @brief Make tensor utilities for mdtensor.
 *
...
//END_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/tensor/resolve_output.hpp
...
//BEGIN_FILE_INCLUDE: /home/runner/work/mdtensor/mdtensor/mdtensor/core/tensor/resolve_reduced_output.hpp
/**
 * @file
 * @brief Output utilities for mdtensor.
 *
...
[[nodiscard]] constexpr auto copy(auto &&in,
    ...
    core::batch<backend>(
        [](auto &&...elems) {
            ufunc::copy_ufunc<calc_t>(std::forward<decltype(elems)>(elems)...);
        },
...
[[nodiscard]] constexpr auto ones(auto &&shape,
    ...
    return full<value_t, backend>(std::forward<decltype(shape)>(shape),
                                  value_t{1}, std::forward<decltype(out)>(out));
}
...
[[nodiscard]] constexpr auto all(auto &&in,
    ...
    if constexpr (sizeof...(axes) == 0) {
        ...
        const bool is_all_true = core::batch_while(
...
[[nodiscard]] constexpr bool all(auto &&in,
                                 where_t &&where = where_t{std::nullopt}) {
    return all<false, backend>(std::forward<decltype(in)>(in),
...
[[nodiscard]] constexpr auto sum(auto &&in,
                                 initial_t &&initial = initial_t{std::nullopt},
                                 where_t &&where = where_t{std::nullopt}) {
    return sum<dtype, false, backend>(std::forward<decltype(in)>(in),
                                      std::index_sequence<>{}, std::nullopt,
...
[[nodiscard]] constexpr auto max(auto &&in,
                                 initial_t &&initial = initial_t{std::nullopt},
                                 where_t &&where = where_t{std::nullopt}) {
    return max<dtype, false, backend>(std::forward<decltype(in)>(in),
                                      std::index_sequence<>{}, std::nullopt,
                                      std::forward<decltype(initial)>(initial),
                                      std::forward<decltype(where)>(where));
}
...
template <typename dtype = void, core::Backend backend = core::Backend::AUTO,
    ...
    requires(!core::integer_sequence_c<initial_t>)
[[nodiscard]] constexpr auto
nanmax(auto &&in, initial_t &&initial = initial_t{std::nullopt},
       where_t &&where = where_t{std::nullopt}) {
    return nanmax<dtype, false, backend>(
